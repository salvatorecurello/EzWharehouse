...
describe("GET /api/skuitems/:rfid", function(){
    it('should get the skuitem with rfid', function(done){
        ...
        .then(function(res){
            ...
            descriptions.should.contain("testSKUchaiForSKUItem");
            id=0;
            for(test of res.body){
                if(test.description=="testSKUchaiForSKUItem"){
                    id=test.id;
                    console.log(id);
                }
            }
        ...
        .then(function(res){
        res.should.have.status(200);
        // questi non li stampa
        console.log(req.body.RFID);
        console.log(req.body.SKUId);
        console.log(req.body.DateOfStock);
    ...
    it('should get the skuitems of sku with id', function(done){
        ...
        .then(function(res){
            ...
            for(test of res.body){
                if(test.description=="testSKUchaiForSKUItem"){
                    id=test.id;
                    console.log(id); // lo stampa
                }
            }
            agent.get("/api/skuitems/sku/"+ id)
            .then(function(res){
            console.log("ciao"); // lo stampa
            res.should.have.status(200); 
            res.body.length.should.be.at.least(1);
            done();
            })
