...
class DAO {
    ...
    createTablesR(sql, i) {
        ...
        }).then(() => this.createTablesR(sql, i + 1));
    }
    createTables() {
        const sql = [];
        ...
        return this.createTablesR(sql, 0).then(() => {
            return new Promise((resolve, reject) => {
                const sql_User = 'INSERT or IGNORE INTO User(NAME, SURNAME, TYPE, PASSWORD, EMAIL) VALUES(?, ?, ?, ?, ?)';
                ...
                users.push({ name: 'nome', surname: 'cognome', type: 'customer', password: password, email: 'user1@ezwh.com' });
                users.push({ name: 'nome', surname: 'cognome', type: 'qualityEmployee', password: password, email: 'qualityEmployee1@ezwh.com' });
                users.push({ name: 'nome', surname: 'cognome', type: 'clerk', password: password, email: 'clerk1@ezwh.com' });
                users.push({ name: 'nome', surname: 'cognome', type: 'deliveryEmployee', password: password, email: 'deliveryEmployee1@ezwh.com' });
                users.push({ name: 'nome', surname: 'cognome', type: 'supplier', password: password, email: 'supplier1@ezwh.com' });
                users.push({ name: 'nome', surname: 'cognome', type: 'manager', password: password, email: 'manager1@ezwh.com' });
                ...
                let items = [];
                items.push({ description: 'description1', price: 2.00, skuid: 1, supplierid: 5 });
                items.push({ description: 'description2', price: 5.00, skuid: 3, supplierid: 2 });
                items.push({ description: 'description3', price: 6.00, skuid: 4, supplierid: 1 });
                items.push({ description: 'description4', price: 1.00, skuid: 2, supplierid: 3 });
                items.push({ description: 'description5', price: 8.00, skuid: 3, supplierid: 2 });

                let ords = [];
                const states = { 'ISSUED': 0, 'ACCEPTED': 1, 'REFUSED': 2, 'CANCELED': 3, 'COMPLETED': 4 };
                ords.push({ issueDate: '20/05/2022', state: states['COMPLETED'], customerID: 1 });
                ords.push({ issueDate: '20/05/2022', state: states['ACCEPTED'], customerID: 1 });
                ords.push({ issueDate: '20/05/2022', state: states['COMPLETED'], customerID: 1 });
                ords.push({ issueDate: '20/05/2022', state: states['ACCEPTED'], customerID: 1 });
                ords.push({ issueDate: '20/05/2022', state: states['ISSUED'], customerID: 1 });
                

                let skus = [];
                skus.push({ description: 'description1', weight: 10, volume: 10, position: 1, availableQuantity: 10, price: 2.00, note: 'note1' });
                skus.push({ description: 'description2', weight: 20, volume: 5, position: 2, availableQuantity: 20, price: 21.00, note: 'note2' });
                skus.push({ description: 'description3', weight: 5, volume: 5, position: 3, availableQuantity: 15, price: 10.00, note: 'note3' });
                skus.push({ description: 'description4', weight: 10, volume: 6, position: 1, availableQuantity: 10, price: 7.00, note: 'note4' });
                skus.push({ description: 'description5', weight: 20, volume: 7, position: 4, availableQuantity: 15, price: 8.00, note: 'note5' });

                let prods = [];
                prods.push({ orderid: 1, skuid: 1, description: 'description1', price: 50.00, qty: 40 });
                prods.push({ orderid: 2, skuid: 2, description: 'description2', price: 70.00, qty: 50 });
                ...
                let testdescriptors = [];
                testdescriptors.push({name:"testdescriptor1", procedureDescription:"proceduredescription1", idSKU: 0})
                testdescriptors.push({name:"testdescriptor2", procedureDescription:"proceduredescription2", idSKU: 1})
                testdescriptors.push({name:"testdescriptor3", procedureDescription:"proceduredescription3", idSKU: 2})
                testdescriptors.push({name:"testdescriptor4", procedureDescription:"proceduredescription4", idSKU: 3})
                testdescriptors.push({name:"testdescriptor5", procedureDescription:"proceduredescription5", idSKU: 4})

                let testresults = [];
                testresults.push({rfid:"11111111111111111111111111111111", idTestDescriptor: 0, Date:dayjs('2021/11/28').unix(), Result:true})
                testresults.push({rfid:"22222222222222222222222222222222", idTestDescriptor: 1, Date:dayjs('2021/11/28').unix(), Result:true})
                testresults.push({rfid:"33333333333333333333333333333333", idTestDescriptor: 2, Date:dayjs('2021/11/28').unix(), Result:false})
                testresults.push({rfid:"44444444444444444444444444444444", idTestDescriptor: 3, Date:dayjs('2021/11/28').unix(), Result:true})
                testresults.push({rfid:"55555555555555555555555555555555", idTestDescriptor: 4, Date:dayjs('2021/11/28').unix(), Result:false})
                ...
                testdescriptors.forEach((testdescriptor) => {
                    this.db.run(sql_testdescriptor, [testdescriptor.name, testdescriptor.procedureDescription, testdescriptor.idSKU], (err) => {
                        if (err)
                            reject(err);
                        else
                            resolve();
                ...
                testresults.forEach((testresult) => {
                    this.db.run(sql_testResult, [testresult.rfid, testresult.idTestDescriptor, testresult.Date, testresult.Result], (err) => {
                        if (err)
                            reject(err);
                        else
                            resolve();
